...
WITH parts AS(
    SELECT unnest((select array_agg(counter) from generate_series(1, 99, 1) AS counter)) as partition_id,
           ...
           unnest((select percentile_cont((select array_agg(s) from generate_series(0.01, 0.99, 0.01) as s)) WITHIN GROUP (ORDER BY longitude) FROM gnaf_202502.address_principals)) as longitude
), parts2 AS (
SELECT 0 AS partition_id, 0.0 AS percentile, min(longitude) - 0.0001 AS longitude FROM gnaf_202502.address_principals
UNION ALL
SELECT * FROM parts
UNION ALL
SELECT 100 AS partition_id, 1.0 AS percentile, max(longitude) - 0.0001 AS longitude FROM gnaf_202502.address_principals
)
SELECT partition_id,
       percentile,
...
WITH merge AS (
    ...
    FROM admin_bdys_202502.commonwealth_electorates as bdy
...
select count(*) from admin_bdys_202502.commonwealth_electorates_analysis;
