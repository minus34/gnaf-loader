...
AWS_PROFILE="minus34"
OUTPUT_FOLDER="/Users/$(whoami)/tmp/geoscape_202502"
OUTPUT_FOLDER_2020="/Users/$(whoami)/tmp/geoscape_202502_gda2020"
GNAF_PATH="/Users/$(whoami)/Downloads/g-naf_nov24_allstates_gda94_psv_1017"
BDYS_PATH="/Users/$(whoami)/Downloads/NOV24_AdminBounds_GDA_94_SHP"
...
aws --profile=${AWS_PROFILE} s3 sync ${OUTPUT_FOLDER} s3://minus34.com/opendata/geoscape-202502 --exclude "*" --include "*.zip" --acl public-read
...
aws --profile=${AWS_PROFILE} s3 sync ${OUTPUT_FOLDER} s3://minus34.com/opendata/geoscape-202502 --exclude "*" --include "*.csv" --acl public-read
...
aws --profile=${AWS_PROFILE} s3 sync ${OUTPUT_FOLDER_2020} s3://minus34.com/opendata/geoscape-202502-gda2020 --exclude "*" --include "*.csv" --acl public-read

echo "---------------------------------------------------------------------------------------------------------------------"
echo "dump postgres schemas to a local folder"
echo "---------------------------------------------------------------------------------------------------------------------"

/Applications/Postgres.app/Contents/Versions/14/bin/pg_dump -Fc -d geo -n gnaf_202502 -p 5432 -U postgres -f "${OUTPUT_FOLDER}/gnaf-202502.dmp" --no-owner
echo "GNAF schema exported to dump file"
/Applications/Postgres.app/Contents/Versions/14/bin/pg_dump -Fc -d geo -n admin_bdys_202502 -p 5432 -U postgres -f "${OUTPUT_FOLDER}/admin-bdys-202502.dmp" --no-owner
...
aws --profile=${AWS_PROFILE} s3 sync ${OUTPUT_FOLDER} s3://minus34.com/opendata/geoscape-202502 --exclude "*" --include "*.dmp" --acl public-read
...
python ${SCRIPT_DIR}/../../spark/xx_export_gnaf_and_admin_bdys_to_geoparquet.py --admin-schema="admin_bdys_202502" --gnaf-schema="gnaf_202502" --output-path="${OUTPUT_FOLDER}/geoparquet"

aws --profile=${AWS_PROFILE} s3 rm s3://minus34.com/opendata/geoscape-202502/geoparquet/ --recursive
aws --profile=${AWS_PROFILE} s3 sync ${OUTPUT_FOLDER}/geoparquet s3://minus34.com/opendata/geoscape-202502/geoparquet --acl public-read
