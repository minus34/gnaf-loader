...
GNAF_PATH="/Users/$(whoami)/Downloads/g-naf_may23_allstates_gda94_psv_1011"
BDYS_PATH="/Users/$(whoami)/Downloads/may23_adminbounds_gda_94_shp"
...
aws --profile=${AWS_PROFILE} s3 sync ${OUTPUT_FOLDER} s3://minus34.com/opendata/geoscape-202305 --exclude "*" --include "*.zip" --acl public-read
...
/Applications/Postgres.app/Contents/Versions/14/bin/pg_dump -Fc -d geo -n gnaf_202305 -p 5432 -U postgres -f "${OUTPUT_FOLDER}/gnaf-202305.dmp" --no-owner
echo "GNAF schema exported to dump file"
/Applications/Postgres.app/Contents/Versions/14/bin/pg_dump -Fc -d geo -n admin_bdys_202305 -p 5432 -U postgres -f "${OUTPUT_FOLDER}/admin-bdys-202305.dmp" --no-owner
...
python ${SCRIPT_DIR}/../../spark/02_export_gnaf_and_admin_bdys_to_s3.py --admin-schema="admin_bdys_202305" --gnaf-schema="gnaf_202305" --output-path="${OUTPUT_FOLDER}/parquet"

aws --profile=${AWS_PROFILE} s3 sync ${OUTPUT_FOLDER}/parquet s3://minus34.com/opendata/geoscape-202305/parquet --acl public-read
