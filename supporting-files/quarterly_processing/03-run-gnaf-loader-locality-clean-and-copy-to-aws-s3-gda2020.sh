...
GNAF_2020_PATH="/Users/$(whoami)/Downloads/g-naf_may25_allstates_gda2020_psv_1019"
BDYS_2020_PATH="/Users/$(whoami)/Downloads/MAY25_AdminBounds_GDA_2020_SHP"

echo "---------------------------------------------------------------------------------------------------------------------"
echo "Run gnaf-loader and locality boundary clean"
echo "---------------------------------------------------------------------------------------------------------------------"

python3 "/Users/$(whoami)/git/minus34/gnaf-loader/load-gnaf.py" --pgport=5432 --pgdb=geo --max-processes=6 --gnaf-tables-path="${GNAF_2020_PATH}" --admin-bdys-path="${BDYS_2020_PATH}" --srid=7844 --gnaf-schema gnaf_202508_gda2020 --admin-schema admin_bdys_202508_gda2020 --previous-gnaf-schema gnaf_202508 --previous-admin-schema admin_bdys_202508
python3 "/Users/$(whoami)/git/iag_geo/psma-admin-bdys/locality-clean.py" --pgport=5432 --pgdb=geo --max-processes=6 --output-path=${OUTPUT_FOLDER_2020} --admin-schema admin_bdys_202508_gda2020

# upload locality bdy files to S3
aws --profile=${AWS_PROFILE} s3 sync ${OUTPUT_FOLDER_2020} s3://minus34.com/opendata/geoscape-202508-gda2020 --exclude "*" --include "*.zip" --acl public-read
...
#python3 /Users/$(whoami)/git/iag_geo/concord/create_concordance_file.py --pgdb=geo --admin-schema="admin_bdys_202508_gda2020" --gnaf-schema="gnaf_202508_gda2020" --output-path=${OUTPUT_FOLDER_2020}
...
/Applications/Postgres.app/Contents/Versions/14/bin/pg_dump -Fc -d geo -n gnaf_202508_gda2020 -p 5432 -U postgres -f "${OUTPUT_FOLDER_2020}/gnaf-202508.dmp" --no-owner
echo "GNAF schema exported to dump file"
/Applications/Postgres.app/Contents/Versions/14/bin/pg_dump -Fc -d geo -n admin_bdys_202508_gda2020 -p 5432 -U postgres -f "${OUTPUT_FOLDER_2020}/admin-bdys-202508.dmp" --no-owner
