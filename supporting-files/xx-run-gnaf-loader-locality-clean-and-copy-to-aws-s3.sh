...
/Applications/Postgres.app/Contents/Versions/11/bin/pg_dump -Fc -d geo -n gnaf_201911 -p 5432 -U postgres -f ${output_folder}/gnaf-201911.dmp --no-owner
echo "GNAF schema exported to dump file"
/Applications/Postgres.app/Contents/Versions/11/bin/pg_dump -Fc -d geo -n admin_bdys_201911 -p 5432 -U postgres -f ${output_folder}/admin-bdys-201911.dmp --no-owner
...
for f in *-201911.dmp;
  do
    aws --profile=${aws_profile} s3 cp --storage-class REDUCED_REDUNDANCY ./${f} s3://minus34.com/opendata/psma-201911/${f};
    aws --profile=${aws_profile} s3api put-object-acl --acl public-read --bucket minus34.com --key opendata/psma-201911/${f}
