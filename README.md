...
* `--pgdb` the database name for Postgres server. This defaults to the `PGDATABASE` environment variable if set, otherwise `psma_202008`.
...
#### Optional Arguments
* `--psma-version` PSMA version number in YYYYMM format. Defaults to current year and last release month. e.g. `202008`.
...
* Local Postgres server: `python load-gnaf.py --gnaf-tables-path="C:\temp\psma_202008\G-NAF" --admin-bdys-path="C:\temp\psma_202008\Administrative Boundaries"` Loads the GNAF tables to a Postgres server running locally. GNAF archives have been extracted to the folder `C:\temp\psma_202008\G-NAF`, and admin boundaries have been extracted to the `C:\temp\psma_202008\Administrative Boundaries` folder.
...
### Process
1. Download [gnaf-202008.dmp](http://minus34.com/opendata/psma-202008/gnaf-202008.dmp) (~1.2Gb)
2. Download [admin-bdys-202008.dmp](http://minus34.com/opendata/psma-202008/admin-bdys-202008.dmp) (~2.7Gb)
