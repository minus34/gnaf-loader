...
* `--pgdb` the database name for Postgres server. This defaults to the `PGDATABASE` environment variable if set, otherwise `psma_201908`.
...
#### Optional Arguments
* `--psma-version` PSMA version number in YYYYMM format. Defaults to current year and last release month. e.g. `201908`.
...
* Local Postgres server: `python load-gnaf.py --gnaf-tables-path="C:\temp\psma_201908\G-NAF" --admin-bdys-path="C:\temp\psma_201908\Administrative Boundaries"` Loads the GNAF tables to a Postgres server running locally. GNAF archives have been extracted to the folder `C:\temp\psma_201908\G-NAF`, and admin boundaries have been extracted to the `C:\temp\psma_201908\Administrative Boundaries` folder.
...
2. Download [admin-bdys-201908.dmp](http://minus34.com/opendata/psma-201908/admin-bdys-201908.dmp) (~2.7Gb)
